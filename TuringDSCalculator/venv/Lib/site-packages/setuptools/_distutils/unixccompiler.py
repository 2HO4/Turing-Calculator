...
def _split_env(cmd):
    """
    For macOS, split command into 'venv' portion (if any)
    and the rest of the linker command.

    >>> _split_env(['a', 'b', 'c'])
    ([], ['a', 'b', 'c'])
    >>> _split_env(['/usr/bin/venv', 'A=3', 'gcc'])
    (['/usr/bin/venv', 'A=3'], ['gcc'])
    """
    pivot = 0
    if os.path.basename(cmd[0]) == "venv":
        pivot = 1
        while '=' in cmd[pivot]:
            pivot += 1
...
def _linker_params(linker_cmd, compiler_cmd):
    ...
    If the LDSHARED venv variable overrides the linker command,
